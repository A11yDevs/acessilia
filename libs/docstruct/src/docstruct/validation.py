...
Ported from backend/pipeline/validators.py. Pure: functions return
structured findings — tuples ``(msgid_constant, kwargs)`` — instead of
translated strings. The consumer (backend) resolves them through its i18n
catalog. msgid constants are the single source of truth.
"""
...
def validate_canonical_document(document: dict[str, Any]) -> list[str]:
    """Validate a canonical document structure and collect structured findings (msgid, kwargs).

    Args:
        document (dict): Canonical document mapping expected to carry the schema_version, id, title, language and sections keys.

    Returns:
        list[str]: Finding tuples (msgid constant + kwargs) describing every structural problem found; empty when the document is well-formed.
    """
    errors: list[str] = []
    if not isinstance(document, dict):
        return [Finding(MSG_CANONICAL_DOC_NOT_OBJECT)]
    for field in ["schema_version", "id", "title", "language", "sections"]:
        if field not in document:
            errors.append(Finding(MSG_FIELD_MISSING, field=field))
    if not isinstance(document.get("sections"), list):
        errors.append(Finding(MSG_SECTIONS_NOT_LIST))
        return errors
    ...
    def walk_blocks(blocks: list[dict[str, Any]]) -> None:
        for block in blocks:
            ...
            if block_id:
                if block_id in ids:
                    errors.append(Finding(MSG_DUPLICATE_ID, block_id=block_id))
                ids.add(block_id)
            ...
            if block.get("type") == "paragraph":
                ...
                if contains_prompt_leak(text):
                    errors.append(
                        Finding(MSG_PROMPT_LEAK_IN_BLOCK, block_id=block_id)
                    )
                if contains_markdown_artifacts(text):
                    errors.append(
                        Finding(MSG_MARKDOWN_IN_BLOCK, block_id=block_id)
                    )
            if block.get("type") == "code":
                code_text = block.get("text", "")
                if _indentation_lost(code_text):
                    errors.append(
                        Finding(MSG_INCONSISTENT_CODE_INDENT, block_id=block_id)
                    )
    ...
    if headings.count(1) > 1:
        errors.append(Finding(MSG_MULTIPLE_H1))
    if headings and headings[0] != 1:
        errors.append(Finding(MSG_MISSING_FIRST_H1))
    if _heading_skips_levels(headings):
        errors.append(Finding(MSG_HEADING_LEVELS_SKIPPED))
    for link in internal_links:
        if link not in ids:
            errors.append(Finding(MSG_INTERNAL_LINK_BROKEN, link=link))
    return errors
...
def validate_export_profile(
    ...
    Returns:
        list[str]: Finding tuples (msgid constant + kwargs) for blocks whose verbosity the profile forbids; empty when all blocks comply.
    """
    profile = OUTPUT_PROFILES.get(profile_name)
    if not profile:
        return [Finding(MSG_UNKNOWN_EXPORT_PROFILE, profile_name=profile_name)]
    ...
    def walk(blocks: list[dict[str, Any]]) -> None:
        for block in blocks:
            if block.get("verbosity", "basic") not in allowed:
                errors.append(
                    Finding(MSG_BLOCK_NOT_ALLOWED_IN_PROFILE, 
                        block_id=block.get("id"),
...
def validate_output_text(text: str, profile_name: str) -> list[str]:
    ...
    Returns:
        list[str]: Finding tuples (msgid constant + kwargs) for each forbidden artifact found; empty when the text is clean.
    """
    errors: list[str] = []
    if contains_prompt_leak(text):
        errors.append(Finding(MSG_PROMPT_LEAK_IN_OUTPUT))
    if profile_name != "html" and contains_markdown_artifacts(text):
        errors.append(Finding(MSG_MARKDOWN_IN_OUTPUT))
    if profile_name == "txt" and re.search(
        r"\[\s*IN[IÍ]CIO DA AUDIODESCRI[CÇ][AÃ]O\s*\]",
        text,
        re.I,
    ):
        errors.append(Finding(MSG_TECHNICAL_METADATA_IN_TXT))
    return errors
...
def audit_canonical_document(document: dict[str, Any]) -> dict[str, list[str]]:
    ...
    if not sections:
        report["BLOCKER"].append(Finding(MSG_DOCUMENT_NO_SECTIONS))
        ...
        for block in blocks:
            if block.get("type") == "image" and not block.get("metadata", {}).get(
                ...
                report["WARNING"].append(
                    Finding(MSG_IMAGE_MISSING_ALT, block_id=block.get("id"))
                )
            if block.get("type") == "table":
                table_id = block.get("id") or Finding(MSG_NO_ID_PLACEHOLDER)
                ...
                if isinstance(table_ast, dict):
                    ...
                    if isinstance(rows, list) and len(rows) > 1:
                        report["WARNING"].append(
                            Finding(MSG_TABLE_LEGACY_FALLBACK, table_id=table_id)
                        )
...
def _validate_table_block(block: dict[str, Any]) -> list[str]:
    ...
    Returns:
        list[str]: Finding tuples (msgid constant + kwargs) for every table structure problem found; empty when the table is well-formed.
    """
    errors: list[str] = []
    block_id = block.get("id") or Finding(MSG_NO_ID_PLACEHOLDER)
    ...
    if rows is not None:
        if not isinstance(rows, list) or not rows:
            errors.append(Finding(MSG_TABLE_ROWS_INVALID, block_id=block_id))
        else:
    ...
    if table_ast is not None:
        ...
        if not isinstance(table_ast.get("body"), list) or not table_ast.get("body"):
            errors.append(Finding(MSG_TABLE_AST_NO_BODY, block_id=block_id))
