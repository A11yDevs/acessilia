...
- Implementation: [backend/agents/orchestrator.py](../backend/agents/orchestrator.py) (`AccessibilityOrchestrator`)
...
1. **Dynamic Smart Orchestration (Future Feature):**
   - Plans to evolve `AccessibilityOrchestrator` into a dynamic router that evaluates document complexity, budget constraints, and SLA to dynamically route tasks between local Ollama models and cloud OpenRouter endpoints.
