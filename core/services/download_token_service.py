...
async def criar_token(output_dir: Path, filename: str) -> str:
    ...
    with _connection_lock:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                "INSERT INTO download_tokens (token, output_dir, filename, zip_path) VALUES (%s, %s, %s, %s)",
...
async def obter_info_token(token: str) -> dict | None:
    with _connection_lock:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT output_dir, filename, criado_em FROM download_tokens WHERE token = %s",
                (token,)
            )
            row = cursor.fetchone()
    if row is None:
